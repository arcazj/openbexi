...
*/
function ob_json_getDate(dateCB, deltaTime, format) {
...
function ob_load_JSON(widget_id, dateCB, timeDelta, widget_type, json, update_according_date) {
    ...
    try {
        if (update_according_date == undefined || update_according_date == "true")
            json = ob_json_getDate(dateCB, timeDelta, json);
    } catch (e) {
    }
    ...
    var tree_name = openbexi_getPageData(pageDoc, "page", widget_id, "tree_name");
 
    //Body: Javascript code
    try {
        openbexi_load_tree_view(widget_id, json, tree_name);
    } catch (e) {
    }
}
