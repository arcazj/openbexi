...
SimileAjax.Graphics.createBubbleForContentAndPoint = function(E, D, B, A, C) {
    if (typeof A != "number") {
        A = 300;
    }
...
SimileAjax.Graphics.createBubbleForPoint = function(B, A, M, Q, H) {
    function S() {
    ...
    var L = function() {
        if (!W._closed) {
            document.body.removeChild(W._div);
            W._doc = null;
            W._div = null;
            W._content = null;
            W._closed = true;
    ...
    var J = function(Y, X, Z, a) {
        Y.style.position = "absolute";
        Y.style.width = Z + "px";
        Y.style.height = a + "px";
        if (R) {
            Y.style.background = "url(" + X + ")";
        } else {
            Y.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + X + "', sizingMethod='crop')";
        }
    };
    ...
    W._div = K;
    W.close = function() {
        SimileAjax.WindowManager.popLayer(V);
    };
    ...
    W.content = I;
    (function() {
        if (B - SimileAjax.Graphics._halfArrowWidth - SimileAjax.Graphics._bubblePadding > 0 && B + SimileAjax.Graphics._halfArrowWidth + SimileAjax.Graphics._bubblePadding < E) {
            var Z = B - Math.round(M / 2) - C.left;
            Z = B < (E / 2) ? Math.max(Z, -(C.left - SimileAjax.Graphics._bubblePadding)) : Math.min(Z, E + (C.right - SimileAjax.Graphics._bubblePadding) - N);
            if ((H && H == "top") || (!H && (A - SimileAjax.Graphics._bubblePointOffset - T > 0))) {
                ...
                if ((H && H == "bottom") || (!H && (A + SimileAjax.Graphics._bubblePointOffset + T < F))) {
                    var X = document.createElement("div");
                    X.style.left = (B - SimileAjax.Graphics._halfArrowWidth - Z) + "px";
                    X.style.top = "0px";
        ...
        if ((H && H == "left") || (!H && (B - SimileAjax.Graphics._bubblePointOffset - N > 0))) {
            var X = document.createElement("div");
            ...
            if ((H && H == "right") || (!H && (B - SimileAjax.Graphics._bubblePointOffset - N < E))) {
                var X = document.createElement("div");
                X.style.left = "0px";
                X.style.top = (A - SimileAjax.Graphics._halfArrowWidth - Y) + "px";
...
SimileAjax.Graphics._FontRenderingContext.prototype.computeSize = function(A) {
    this._elmt.innerHTML = A;
