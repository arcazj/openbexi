...
Timeline._Band = function(timeline, bandInfo, index) {
    ...
    //Update for OpenBEXI: Add mobile gesture
    ...
    //End Update for OpenBEXI: Add mobile gesture
};
...
Timeline._Band.prototype._onKeyDown = function(keyboardInput, evt, target) {
    if (!this._dragging) {
        switch (evt.keyCode) {case 27:break;
...
Timeline.OriginalEventPainter.prototype._paintEventTape = function(evt, iconTrack, startPixel, endPixel, color, opacity, metrics, theme, tape_index) {
    ...
    //OpenBEXI update - remove opacity set up because opacity may be  already defined in css files
    //SimileAjax.Graphics.setOpacity(tapeDiv, opacity);
    this._eventLayer.appendChild(tapeDiv);
    return{left:startPixel,top:top,width:tapeWidth,height:tapeHeight,elmt:tapeDiv};
};
...
Timeline.createBandInfo = function(params) {
    ...
    if ("eventPainter" in params) {
        ...
        switch (layout) {case"overview":eventPainter = new Timeline.OverviewEventPainter(eventPainterParams);
            break;
